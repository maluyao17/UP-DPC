...
class Dpcpp:
    ...
    def final_cluster(self):
        ...
        for i in range(0, 16):
            ...
            min_idx,__ = kmeans.fit_predict(data, centroids=self.kmeans_center)
...
def get_prototype(data):
    ...
    if data.dtype == torch.int:
        fit = 1
    elif data.dtype == torch.double:
        fit = 8
    elif data.dtype == torch.float:
        fit = 4
    elif data.dtype == torch.half:
        fit = 2
    Eud_memory = data.shape[0]*p*fit
    s = math.ceil(((memory - Eud_memory)*0.01*fit/memory)*data.shape[0])
    ...
    for b in range(ratio):
        data_tmp = data[(int)(b*sub_batch_size):((int)((b+1)*sub_batch_size)),:]
        ...
        data_to_kms[(int)(b*sub_batch_size):((int)((b+1)*sub_batch_size)),:] = data_to_kms_
    ...
    return kms_center, data_to_kms, N, s, p
