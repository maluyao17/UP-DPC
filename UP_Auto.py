...
class Dpcpp:
    ...
    def __init__(self, data, label, l, r=100):
        self.data = data
        self. label = label
        # self.s = s
        # self.p = p
    ...
    def fit(self):
        ...
        starttime_NDH = datetime.datetime.now()
        nneigh, sim, sort_rho_idx = self.simNeigh_creator(similarity_mat, rho)
        endtime = datetime.datetime.now()
        print("NDH:"+str(endtime - starttime_NDH))
    ...
    def sample_Data(self):
        ...
        N = self.data.shape[0]
        memory = self.remaining_memory()
        self.p = 500
        # self.p = random.randint(5,6)*100
        if self.data.dtype == torch.int:
            fit = 1
        elif self.data.dtype == torch.double:
            fit = 8
        elif self.data.dtype == torch.float:
            fit = 4
        elif self.data.dtype == torch.half:
            fit = 2
        Eud_memory = self.data.shape[0]*self.p*fit
        if N > 100000:
            self.s = math.ceil(((memory - Eud_memory)*0.01*fit/memory)*self.data.shape[0])
        else:
            self.s = math.ceil(((memory - Eud_memory)*0.1*fit/memory)*self.data.shape[0])
        print((memory - Eud_memory)*0.1*fit/memory)
              
        if self.s < 1000 and N > 1000:
            self.s = 1000
        # self.s = 50000
        self.cluster = torch.Tensor([-1] * self.p).to(device)

        # print('memory:'+str(self.remaining_memory()))
        # print('N size:'+str(self.data.shape[0]*self.data.shape[1]*4))
        self.N = N
        ...
        print('memory:'+str(self.remaining_memory()))
        # print('shuffle size:'+str(kmeans_data.shape[0]*kmeans_data.shape[1]*4))
        ...
        tmp_index = data_to_kms.min(axis=1).indices       # Index of the minimum distance from each input sample to the Kmeans cluster center
